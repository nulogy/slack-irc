...
class Bot {
  constructor(options) {
    ...
    this.shouldSendToSlack = _.get(options, 'sendToSlack', true);
    this.shouldSendToIRC = _.get(options, 'sendToIRC', true);
  ...
  sendToIRC(message) {
    ...
    if (!this.shouldSendToIRC) {
      logger.debug(
        `Muted all messages to IRC, would have sent: "<${message.author}>: ${message.text}"`
      );
      return;
    }
  ...
  sendToSlack(author, channel, text) {
    ...
    if (!this.shouldSendToSlack) {
      logger.debug(`Muted all messages to slack, would have sent: "<${author}>: ${text}"`);
      return;
    }
